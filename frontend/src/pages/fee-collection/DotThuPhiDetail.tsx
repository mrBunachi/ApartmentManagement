...
const DotThuPhiDetail = () => {
  ...
  // Đóng góp modal
  const [showDongGopModal, setShowDongGopModal] = useState(false);
  const [households, setHouseholds] = useState<any[]>([]);
  const [dongGopData, setDongGopData] = useState({
    MAHOKHAU: 0,
    SOTIENDADONG: 0,
    HINHTHUC: 'Tiền mặt',
    GHICHU: '',
  ...
  const fetchHouseholds = async () => {
    try {
      const response = await householdService.getAll({ ACTIVATE: true });
  ...
  const openDongGopModal = async () => {
    ...
    setShowDongGopModal(true);
  };

  const handleDongGop = async () => {
    try {
      if (!dongGopData.MAHOKHAU || dongGopData.SOTIENDADONG <= 0) {
        toast.error('Vui lòng chọn hộ khẩu và nhập số tiền hợp lệ');
        return;
      }
      ...
      toast.success('Tạo phiếu đóng góp thành công');
      setShowDongGopModal(false);
      fetchDongGop();
    } catch (error: any) {
      toast.error(error.message || 'Lỗi khi tạo đóng góp');
    }
  };
  ...
  return (
    <div className="p-6">
      ...
      {showDongGopModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <h2 className="text-xl font-bold mb-4">Tạo phiếu đóng góp</h2>
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  value={dongGopData.MAHOKHAU}
                  onChange={(e) => setDongGopData({ ...dongGopData, MAHOKHAU: parseInt(e.target.value) })}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2"
                >
                  <option value={0}>-- Chọn hộ khẩu --</option>
                  {households.map((hh) => (
                    <option key={hh.MAHOKHAU} value={hh.MAHOKHAU}>
                      {hh.MAPHONG} - {hh.THONGTINCHUHO?.HOTEN || 'N/A'}
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div>
                ...
                <input
                  type="number"
                  value={dongGopData.SOTIENDADONG}
                  onChange={(e) => setDongGopData({ ...dongGopData, SOTIENDADONG: parseFloat(e.target.value) })}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2"
              ...
              <div>
                ...
                <select
                  value={dongGopData.HINHTHUC}
                  onChange={(e) => setDongGopData({ ...dongGopData, HINHTHUC: e.target.value })}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2"
                >
                  <option value="Tiền mặt">Tiền mặt</option>
                  <option value="Chuyển khoản">Chuyển khoản</option>
              ...
              <div>
                ...
                <textarea
                  value={dongGopData.GHICHU}
                  onChange={(e) => setDongGopData({ ...dongGopData, GHICHU: e.target.value })}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2"
                  rows={3}
            ...
            <div className="flex gap-3 mt-6">
              <button
                onClick={handleDongGop}
                className="flex-1 bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700"
              >
                Xác nhận
              </button>
              <button
                onClick={() => setShowDongGopModal(false)}
                className="flex-1 bg-gray-300 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-400"
              >
                Hủy
              </button>
